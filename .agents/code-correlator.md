...
- `~/scm/repo/svn/CLOUD/branches/CURRENT/kube/helm` — ★ **배포 helm 차트 (SVN 형상관리)**. 모든 차트가
...
- **helm 차트 분석 전 반드시 src_repo_log 로 동기화 상태를 먼저 확인하라**: 출력의
  '동기화 상태'에 `*` 항목이 있으면 작업복사본이 저장소 HEAD보다 오래된 것이다.
  이 경우 답변 서두에 "SVN 작업복사본이 구버전(svn update 필요)"을 명시하고,
  분석이 구버전 차트 기준임을 전제로 서술하라.
